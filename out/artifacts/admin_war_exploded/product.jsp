...
<head>
    ...
    <script>
        ...
        function selall(o) {
            var a = document.getElementsByName('pids');
...
<div class="container clearfix">
    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list" style="font-size: 17px">
                ...
                <span class="crumb-name">商品管理</span>
            </div>
        </div>
        ...
        <div class="search-wrap">
            <div class="search-content">
                <form action="${pageContext.request.contextPath}/productList" method="post">
                    <input class="common-text" name="pname" placeholder=" 商品名" type="text" style="margin-left: 30px;outline-style: none;border: 1px solid #ccc;height: 29px;padding: 4px 11px;width: 142px;font-size: 15px;font-weight: 500;margin-top: -2px;">
                    <input class="common-text" name="pclass" placeholder=" 商品类别" type="text" style="margin-left: 30px;outline-style: none;border: 1px solid #ccc;height: 29px;padding: 4px 11px;width: 142px;font-size: 15px;font-weight: 500;margin-top: -2px;">
    ...
    <div id="register" class="result-wrap">
        <%--删除商品--%>
        <form action="${pageContext.request.contextPath}/deleteSelectedProduct" id="myForm" method="post">
            <div class="result-title">
                <div class="result-list">
                    <a href="${pageContext.request.contextPath}/productAdd.jsp"><i class="icon-font"></i>新增商品</a>
            ...
            <div class="result-content">
                <table class="result-tab" style="text-align:center !important; width: 100%">
                    ...
                    <c:forEach var="product" items="${pageInfo.list}">
                        <tr>
                            <td class="tc"><input name="pids" value="${product.pid}" type="checkbox"></td>
                            <td>${product.pid}</td>
                            <td><img src="${pageContext.request.contextPath}/statics/img/product/${product.picture}" width="80" height="80"></td>
                            <td>${product.pname}</td>
                            <td>${product.pclass}</td>
                            <td>${product.price}</td>
                            <td>${product.stock}</td>
                            <td>${product.pdes}</td>
                            <td>
                                <a class="link-update" href="${pageContext.request.contextPath}/queryProductByPid?pid=${product.pid}" style="color: deepskyblue">修改</a>
                                |
                                <a class="link-del" href="javascript:Delete('你确定要删除商品 [ ${product.pname} ] 吗？', '${pageContext.request.contextPath}/deleteProduct?pid=${product.pid}')" style="color: red">删除</a>
                            </td>
                        </tr>
                    </c:forEach>
                </table>
...
<script>
    function before(page) {
        if (page - 1 < 1) {
            ...
            location.href = "${pageContext.request.contextPath}/productList?startPage="+ (page - 1) + "&pageSize=5";
        }
    }
